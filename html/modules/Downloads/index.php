...
function getparentlink($parentid,$title) {
    ...
    $result=sql_query("select cid, title, parentid from ".$prefix."_downloads_categories where cid=$parentid", $dbi);
    list($cid, $ptitle, $pparentid) = sql_fetch_row($result, $dbi);
    if ($ptitle!="") $title="<a href=modules.php?op=modload&name=Downloads&d_op=viewdownload&cid=$cid>$ptitle</a>/".$title;
    if ($pparentid!=0) {
...
function index() {
	...
	while(list($cid, $stitle) = sql_fetch_row($result2, $dbi)) {
		...
		echo ",&nbsp;";
            ...
            $cresult2 = sql_query("select * from ".$prefix."_downloads_downloads where cid='$cid'", $dbi);
            $cnumrows2 = sql_num_rows($cresult2, $dbi);
	    echo "<font class=\"content\"><a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;cid=$cid\">$stitle</a> ($cnumrows2)</font>";
	    $space++;
	}
	if ($count<1) {
	    echo "</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
...
function AddDownload() {
    ...
    if (is_user($user) || $downloads_anonadddownloadlock != 1) {
    	...
    	echo "<form method=\"post\" action=\"modules.php?name=Downloads&d_op=Add\">"
...
function NewDownloadsDate($selectdate) {
    ...
    $result = sql_query("select lid, cid, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments, filesize, version, homepage from ".$prefix."_downloads_downloads where date LIKE '%$newdownloadDB%' order by title ASC", $dbi);
...
function TopRated($ratenum, $ratetype) {			
    ...
    $result = sql_query("select lid, cid, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments, filesize, version, homepage from ".$prefix."_downloads_downloads where downloadratingsummary != 0 and totalvotes >= $downloadvotemin order by downloadratingsummary DESC limit 0,$topdownloads ", $dbi);
    echo "<tr><td>";
    while(list($lid, $cid, $title, $description, $time, $hits, $downloadratingsummary, $totalvotes, $totalcomments, $filesize, $version, $homepage) = sql_fetch_row($result, $dbi)) {
	$downloadratingsummary = number_format($downloadratingsummary, $mainvotedecimal);
...
function MostPopular($ratenum, $ratetype) {
    ...
    $result = sql_query("select lid, cid, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments, filesize, version, homepage from ".$prefix."_downloads_downloads order by hits DESC limit 0,$mostpopdownloads ", $dbi);
    echo "<tr><td>";
    while(list($lid, $cid, $title, $description, $time, $hits, $downloadratingsummary, $totalvotes, $totalcomments, $filesize, $version, $homepage) = sql_fetch_row($result, $dbi)) {
	$downloadratingsummary = number_format($downloadratingsummary, $mainvotedecimal);
	$title = stripslashes($title);
...
function viewdownload($cid, $min, $orderby, $show) {
	...
	$title="<a href=modules.php?name=Downloads>"._MAIN."</a>/$title";
    ...
    while(list($cid2, $title2, $cdescription2) = sql_fetch_row($result2, $dbi)) {
        ...
        $cnumrows = sql_num_rows($cresult, $dbi);
	echo "<td><font class=\"option\"><strong><big>·</big></strong> <a href=\"modules.php?op=modload&name=Downloads&d_op=viewdownload&amp;cid=$cid2\"><b>$title2</b></a> ($cnumrows)</font>";
	...
	while(list($cid3, $title3) = sql_fetch_row($result3, $dbi)) {
		...
		echo ",&nbsp;";
            ...
            $cnumrows2 = sql_num_rows($cresult2, $dbi);
	    echo "<font class=\"content\"><a href=\"modules.php?op=modload&name=Downloads&d_op=viewdownload&amp;cid=$cid3\">$title3</a> ($cnumrows2)</font>";
	    $space++;
	}
	if ($count<1) {
	    echo "</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
    ...
    $result=sql_query("select lid, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments, filesize, version, homepage from ".$prefix."_downloads_downloads where cid=$cid order by $orderby limit $min,$perpage ", $dbi);
    $fullcountresult=sql_query("select lid, title, description, date, hits, downloadratingsummary, totalvotes, totalcomments from ".$prefix."_downloads_downloads where cid=$cid", $dbi);
...
function viewsdownload($sid, $min, $orderby, $show) {
    ...
    $count = 0;
    while(list($cid2, $title2, $cdescription2) = sql_fetch_row($result2, $dbi)) {
	echo "<td><font class=\"option\"><strong><big>·</big></strong> <a href=\"modules.php?op=modload&name=Downloads&d_op=viewdownload&amp;cid=$cid2\"><b>$title2</b></a></font>";
	categorynewdownloadgraphic($cid2);
	if ($cdescription2) {
	    echo "<font class=\"content\">$cdescription2</font><br>";
	} else {
	...
	while(list($cid3, $title3) = sql_fetch_row($result3, $dbi)) {
	    ...
	    echo "<font class=\"content\"><a href=\"modules.php?op=modload&name=Downloads&d_op=viewdownload&amp;cid=$cid3\">$title3</a></font>";
	    $space++;
	}
	if ($count<1) {
	    echo "</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>";
	...
	.""._TITLE." (<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=titleA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=titleD\">D</a>)"
	." "._DATE." (<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=dateA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=dateD\">D</a>)"
	." "._RATING." (<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=ratingA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=ratingD\">D</a>)"
        ." "._POPULARITY." (<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=hitsA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;orderby=hitsD\">D</a>)"
	."<br><b>"._RESSORTED.": $orderbyTrans</b></font></center><br><br>";
	...
	echo "<br><br>"
        ...
        $prev=$min-$perpage;
        if ($prev>=0) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;min=$prev&amp;orderby=$orderby&amp;show=$show\">"
    		." &lt;&lt; "._PREVIOUS."</a> ]</b> ";
      	} 	
        $counter = 1;
        $currentpage = ($max / $perpage);
		...
		echo "<a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;min=$mintemp&amp;orderby=$orderby&amp;show=$show\">$counter</a> ";
	    }
            $counter++; 	
        }    	
        $next=$min+$perpage;
        if ($x>=$perpage) {
    	    echo "&nbsp;&nbsp;<b>[ <a href=\"modules.php?name=Downloads&d_op=viewdownload&amp;sid=$sid&amp;min=$max&amp;orderby=$orderby&amp;show=$show\">"
    		." "._NEXT." &gt;&gt;</a> ]</b> ";
        }
    }
    echo "</td></tr></table>";
...
function search($query, $min, $orderby, $show) {   
    ...
    if ($query != "") {
		...
		while(list($cid, $stitle) = sql_fetch_row($result2, $dbi)) {
    		...
    		echo "<strong><big>·</big></strong>&nbsp;<a href=\"modules.php?op=modload&name=Downloads&d_op=viewdownload&amp;cid=$cid\">$title3</a> ($numrows)<br>";
		}
	echo "<br><table width=\"100%\" bgcolor=\"$bgcolor2\"><tr><td><font class=\"option\"><b>"._UDOWNLOADS."</b></font></td></tr></table>";
    	$orderbyTrans = convertorderbytrans($orderby);
    	echo "<center><font class=\"content\">"._SORTDOWNLOADSBY.": "
    	    .""._TITLE." (<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=titleA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=titleD\">D</a>) "
    	    .""._DATE." (<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=dateA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=dateD\">D</a>) "
    	    .""._RATING." (<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=ratingA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=ratingD\">D</a>) "
    	    .""._POPULARITY." (<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=hitsA\">A</a>\<a href=\"modules.php?name=Downloads&d_op=search&amp;query=$the_query&amp;orderby=hitsD\">D</a>)"
...
function brokendownloadS($lid, $modifysubmitter) {
    include("modules/Downloads/d_config.php");
    global $prefix, $dbi, $user, $anonymous, $cookie;
    if(is_user($user)) {
	$user2 = base64_decode($user);
   	$cookie = explode(":", $user2);
	cookiedecode($user);
